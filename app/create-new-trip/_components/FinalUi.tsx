...
function FinalUi({ viewTrip, tripReady, tripPlan }: { viewTrip: () => void; tripReady: boolean; tripPlan: any }) {
  ...
  const handleClick = () => {
    ...
    router.push(
      `/trip?destination=${encodeURIComponent(tripPlan.destination)}&hotel=${encodeURIComponent(tripPlan.hotel)}&image=${encodeURIComponent(tripPlan.image || 'https://source.unsplash.com/1600x900/?' + tripPlan.destination)}`
    );
  };
